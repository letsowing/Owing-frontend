...
export const AIWindow = ({
  ...
  return (
    <div
      className={`z-999 absolute right-10 top-20 h-3/4 w-1/3 rounded-lg border border-lightgray bg-white pl-1 text-darkgray transition-all duration-300 ease-in-out ${isOpen ? 'translate-y-0 opacity-100' : 'pointer-events-none translate-y-4 opacity-0'} `}
    >
      <div className="flex h-full">
        <div className="h-2/3 flex-1">{children}</div>
        {selectedFeature ? (
