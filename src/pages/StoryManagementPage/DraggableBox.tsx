      ...
      {isEditing ? (
        <>
          <input
            ...
            className="mb-2 rounded rounded-md border p-2 dark:bg-verydarkblack"
          />
          <textarea
            placeholder="줄거리를 입력할 수 있습니다."
            value={editedDescription || ''}
            onChange={(e) => setEditedDescription(e.target.value)}
            className="flex-grow resize-none rounded-md border p-2 dark:bg-verydarkblack"
          />
          <div className="mb-2 me-2 ml-auto mt-4 flex flex-row items-center space-x-2">
            <button
              onClick={handleCancel}
              className="text-md h-7 px-4 text-darkgray hover:rounded-md hover:bg-darkgray hover:text-white dark:text-coldbeige dark:hover:bg-coldbeige dark:hover:text-darkgray"
            >
              취소
            </button>
            <button
        ...
        <>
          ...
          <button
            onClick={handleEdit}
            className="text-md mb-2 me-2 ml-auto mt-auto h-7 from-redorange to-orange px-4 text-redorange hover:rounded-[10px] hover:bg-gradient-to-r hover:text-white dark:from-blue dark:to-skyblue dark:text-blue dark:hover:text-coldbeige"
          >
            줄거리 수정
          </button>
        </>
